...
for model in config.modelname:
	device_count_fetch = fetch_sis_api('equipment', {'ownercode' : config.ownercode, 'modelname': model, 'page[size]' : 1})
	...
	while i <= device_pages:
		device_fetch = fetch_sis_api('equipment', {'ownercode' : config.ownercode, 'modelname': model, 'page[number]' : i})
		i += 1
		for device in device_fetch['data']:
			# Check for public IP to use
			ipv4address = ""
			for address in device["attributes"]['equipips']:
				if ipv4address == "" or ipaddress.ip_address('192.168.0.1').is_global:
			...
			if ipv4address != "":	
				try: 
					equipment[device["id"]] = Device(
						...
						model = model,
						address = ipv4address,
						lookupcode = device['relationships']['equipinstalls']['data'][0]['lookupcode'],
						host_name = device['relationships']['equipinstalls']['data'][0]['lookupcode']+"_"+model.replace(" ","_")
						)
				except:
					pass
...
site_count_fetch = fetch_sis_api('site-epochs', {'netcode' : config.ownercode, 'page[size]' : 1})
site_count = site_count_fetch['meta']['pagination']['count']
site_pages = math.ceil(site_count/config.page_size)
i = 1
# Fetch each page 
while i <= site_pages:
	site_fetch = fetch_sis_api('site-epochs', {'netcode' : config.ownercode, 'page[number]' : i})
	i += 1
	# Populate sites
	...
	if os.path.isfile(file_path):
		for line in fileinput.FileInput(file_path,inplace=1):
			if line.startswith("\t_id"):
				...
				if equipment[item].lookupcode in sites and sites[equipment[item].lookupcode].latitude != "":
					line="\t_latitude\t"+str(sites[equipment[item].lookupcode].latitude)+"\n"
			elif line.startswith("\t_longitude"):
				if equipment[item].lookupcode in sites and sites[equipment[item].lookupcode].longitude != "":
					line="\t_longitude\t"+str(sites[equipment[item].lookupcode].longitude)+"\n"				
			print(line,end="")
	else:
		# Create new file
		f = open(file_path, "w")
		...
		if equipment[item].lookupcode in sites:
			f.write("\t_latitude\t"+str(sites[equipment[item].lookupcode].latitude)+"\n")
			f.write("\t_longitude\t"+str(sites[equipment[item].lookupcode].longitude)+"\n")
		else:
			f.write("\t_latitude\n")
			f.write("\t_longitude\n")
